...
namespace FG.CheckoutAndBuild2.Common
{
	...
	public static class Check
	{
		...
		public async static Task TryCatchAsync<TResult, TException>(Func<TResult> block, Func<TException, Exception> onException = null)
			where TException : Exception
		{
			await Task.Run(() => TryCatch<TResult, TException>(block, onException));
		}

		public async static Task TryCatchAsync<TException>(Action block, Func<TException, Exception> onException = null)
			where TException : Exception
		{
			await Task.Run(() => TryCatch<TException>(block, onException));
		}

		public async static Task<TResult> TryCatchAsync<TResult, TException>(Task<TResult> task, 
		...
		public async static Task TryCatchAsync<TException>(Task block, Func<TException, Exception> onException = null)
			where TException : Exception
		{
			try
			{
