...
namespace FG.CheckoutAndBuild2.Common
{
	...
	internal class PropertyGridSearchExtenter
	{
		...
		private string searchText { get { return "Search content"; } }
		...
		private void SearchBoxTextChanged(PropertyGrid control, ToolStripTextBox tb)
		{
			...
			if (!string.IsNullOrWhiteSpace(tb.Text) && tb.Text != searchText)
			{
				...
				{
					var customClass = new CustomClass(string.Empty);
					foreach (
						PropertyInfo propertyInfo in propObject.GetType().GetProperties().Where(info => info.Name.ToLower().Contains(tb.Text.ToLower())))
					{
						try
						{
