...
namespace FG.CheckoutAndBuild2.Common
{
    public class PausableCancellationTokenSource : CancellationTokenSource
    {
        ...
        {
            var context = System.Threading.Tasks.TaskScheduler.FromCurrentSynchronizationContext();
            Task.Delay(delay).ContinueWith(task => Resume(), context);
        }
        ...
        {
            get { return typeof(CancellationTokenSource).GetField("m_linkingRegistrations", flags)?.GetValue(this) as CancellationTokenRegistration[]; }
